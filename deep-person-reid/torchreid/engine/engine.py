...
class Engine(object):
    ...
    def _evaluate(
        ...
        print('Time elapsed: {:.4f} min'.format(1/batch_time.avg))
        ...
        # Store last evaluation results for external logging/consumption
        try:
            self.last_eval_results = {
                'dataset_name': dataset_name,
                'mAP': float(mAP),
                'cmc': [float(x) for x in cmc.tolist()],
                'speed_sec_per_batch': float(batch_time.avg),
                'time_elapsed_min': float(1 / batch_time.avg),
                'ranks': ranks,
            }
        except Exception:
            # Be resilient to any unexpected types
            self.last_eval_results = {
                'dataset_name': dataset_name,
                'mAP': mAP,
                'cmc': cmc,
                'speed_sec_per_batch': batch_time.avg,
                'time_elapsed_min': 1 / batch_time.avg,
                'ranks': ranks,
            }

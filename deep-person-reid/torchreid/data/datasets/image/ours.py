...
class OURS(ImageDataset):
    ...
    def __init__(self, root='', split_id=0, **kwargs):
        ...
        self.data_dir = osp.join(self.dataset_dir, 'reid_output')
        self.split_path = osp.join(self.dataset_dir, 'output.json')
        ...
        super(OURS, self).__init__(train, query, gallery, **kwargs)
    ...
    def get_pid2label(self, img_names, mode):
        pid_container = set()
    ...
    def parse_img_names(self, img_names, mode, pid2label=None):
        data = []
        ...
        for item in img_names:
            ...
            if pid2label is not None:
                pid = pid2label[pid]
            camid = int(img_name[6]) - 1 # 0-based
    ...
    def process_split(self, split):
        train_pid2label = self.get_pid2label(split, mode='train')
        train = self.parse_img_names(split, mode='train', pid2label=train_pid2label)
        query = self.parse_img_names(split, mode='query')

        gallery = self.parse_img_names(split, mode='gallery')

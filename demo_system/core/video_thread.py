...
class VideoThread(QThread):
    ...
    def __init__(self, camera_id=0, rotate=True, display_width=1280, display_height=720, 
                 inference_width=640, inference_height=360):
        super().__init__()
        self.camera_id = camera_id
        self.rotate = rotate
        self._run_flag = True
        self.buffer_size = 2  # Buffer size, set to 1 to avoid delay
    ...
    def auto_detect_orientation(self, file_path):
        ...
        try:
            ...
            # Get original video size information
    ...
    def run(self):
        """Main thread loop - optimized to reduce frame copying"""
        ...
        if self.is_camera:
            ...
            # Disable audio capture - only capture video frames
            try:
                # Try to disable audio capture if supported
                self.cap.set(cv2.CAP_PROP_AUDIO_STREAM, -1)  # Disable audio stream
                print("Audio capture disabled - video only mode")
            except Exception as e:
                print(f"Note: Audio disable not supported on this camera: {e}")
                # Continue without audio - this is expected behavior
            ...
            # Disable audio capture for video files - only capture video frames
            try:
                # Try to disable audio stream if supported
                self.cap.set(cv2.CAP_PROP_AUDIO_STREAM, -1)  # Disable audio stream
                print("Audio capture disabled for video file - video only mode")
            except Exception as e:
                print(f"Note: Audio disable not supported for this video file: {e}")
                # Continue without audio - this is expected behavior
        ...
        # Pre-allocate inference frame buffer to avoid repeated allocation
        inference_buffer = np.zeros((self.inference_height, self.inference_width, 3), dtype=np.uint8)
        
        # Run flag
        while self._run_flag:
            ret, frame = self.cap.read()
            if ret:
                # Apply transformations directly on original frame to avoid unnecessary copying
                ...
                # Create inference frame (low resolution), reuse buffer
                cv2.resize(processed_frame, (self.inference_width, self.inference_height), dst=inference_buffer)
                
                # Notify main window that new inference frame is available
                if hasattr(self, 'main_window') and hasattr(self.main_window, 'set_inference_frame'):
                    # Create copy of inference frame for async processor
                    inference_copy = inference_buffer.copy()
                    self.main_window.set_inference_frame(inference_copy)
                ...
                # Send display frame (processed frame)
                self.change_pixmap_signal.emit(processed_frame, fps_display)
            else:

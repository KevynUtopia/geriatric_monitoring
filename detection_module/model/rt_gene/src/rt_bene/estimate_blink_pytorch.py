...
from model.rt_gene.src.rt_gene.download_tools import download_blink_pytorch_models, md5
from model.rt_gene.src.rt_bene.estimate_blink_base import BlinkEstimatorBase
from model.rt_gene.src.rt_bene.blink_estimation_models_pytorch import BlinkEstimationModelResnet18, BlinkEstimationModelVGG16, BlinkEstimationModelVGG19, BlinkEstimationModelResnet50, BlinkEstimationModelDenseNet121
import os
import cv2
import torch
...
class BlinkEstimatorPytorch(BlinkEstimatorBase):
    ...
    "cde99055e3b6dcf9fae6b78191c0fd9b", "67339ceefcfec4b3b8b3d7ccb03fadfa", "e5de548b2a97162c5e655259463e4d23", "7c228fe7b95ce5960c4c5cae8f2d3a09", "0a0d2d066737b333737018d738de386f")):
        ...
        model_files = ['/home/wzhangbu/HavenofHope/code/facial_analysis/weights/blink_model_pytorch_resnet18_allsubjects1.model']
        # check md5 hashes
        ...
        self._transform = transforms.Compose([
            # lambda x: cv2.resize(x, dsize=self.input_size, interpolation=cv2.INTER_CUBIC),
                                              transforms.ToTensor(),
    ...
    def inputs_from_images(self, left, right):
        left =  cv2.resize(left, self.input_size, interpolation=cv2.INTER_CUBIC)
        right = cv2.resize(right, self.input_size, interpolation=cv2.INTER_CUBIC)

...
class BlinkEstimatorBase(object):
    def __init__(self, device_id_blink, model_files, threshold, input_size):
        ...
        img_input_l = tf.keras.Input(shape=input_size + (3, ),
                                     name='img_input_L')
        img_input_r = tf.keras.Input(shape=input_size + (3, ),
                                     name='img_input_R')
        ...
        if len(models) == 1:
            ...
            self.model = tf.keras.Model(inputs=[img_input_r, img_input_l],
        ...
        self.predict(np.zeros((1, ) + input_size + (3, )),
                     np.zeros((1, ) + input_size + (3, )))
    ...
    def predict(self, left_eyes, right_eyes):
        with self.graph.as_default():
            tf.compat.v1.keras.backend.set_session(self.sess)
            x = [np.array(right_eyes),
                 np.array(left_eyes)]  # model expects this order!
            p = self.model.predict(x, verbose=0)
            blinks = p >= self.threshold
            return p, blinks


class BlinkEstimatorImageLandmark(BlinkEstimatorBase):
    def __init__(
        self,
        threshold,
        input_size,
        crop_offset,
        model_path):
        super(BlinkEstimatorImageLandmark,
              self).__init__("/GPU:2", model_path, threshold, input_size)
    ...
    def estimate(self, images, landmarks):
        ...
        for image, landmark in zip(images, landmarks):
            ...
            left_images.append(self.resize_img(crop(image, left_rect)))
            right_images.append(self.resize_img(crop(image, right_rect)))

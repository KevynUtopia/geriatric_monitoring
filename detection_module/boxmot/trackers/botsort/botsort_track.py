...
class STrack(BaseTrack):
    ...
    def activate(self, kalman_filter, frame_id, image=None):
        """Activate a new track."""
        self.kalman_filter = kalman_filter
        self.id = self.next_id()
        self.mean, self.covariance = self.kalman_filter.initiate(self.xywh)

        # crop the image using self.xywh and self.image
        # self.save_image(image, utils.OUTPUT_FOLDER)
    ...
    def re_activate(self, new_track, frame_id, new_id=False, image=None):
    ...
    def save_image(self, image, path):
        x1, y1, x2, y2 = xywh2xyxy(self.xywh)
        x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
        cv2.imwrite(f"{path}/id_{self.id}.jpg", image[y1:y2, x1:x2])

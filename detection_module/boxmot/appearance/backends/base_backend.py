...
class BaseModelBackend:
    ...
    def get_crops(self, xyxys, img):
        ...
        for i, box in enumerate(xyxys):
            ...
            # cv2.imwrite(f'path_to_your_root/results/reid/{i}.jpg', crop)
    ...
    @torch.no_grad()
    def get_reid_features(self, paths):
        resize_dims = (128, 256)
        num_crops = len(paths)
        interpolation_method = cv2.INTER_LINEAR
        mean_array = torch.tensor([0.485, 0.456, 0.406], device=self.device).view(1, 3, 1, 1)
        ...
        for i, path in enumerate(paths):
            ...
            crop = cv2.resize(img, resize_dims, interpolation=interpolation_method)
            crop = cv2.cvtColor(crop, cv2.COLOR_BGR2RGB)

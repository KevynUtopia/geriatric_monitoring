...
Copied from demo_system.models.pose_detector and namespaced under
demo_system_backend.models so that demo_system_backend can run independently.
"""
...
class PoseDetector(BaseDetector):
    ...
    def detect(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        try:
                ...
                if result.keypoints is not None and len(result.keypoints.data) > 0:
                    ...
                    for idx, person_keypoints in enumerate(keypoints_data):
                        if len(person_keypoints) > 0:
                            kpts_xy = person_keypoints[:, :2]
                            kpts_conf = person_keypoints[:, 2]

                            valid_mask = kpts_conf > self.conf_threshold
                            kpts_xy[~valid_mask] = [0, 0]
                            ...
                            if boxes_xyxy is not None and len(boxes_xyxy) > idx:
                                ...
                                box_scores_list.append(
                                    float(boxes_conf[idx])
                                    if boxes_conf is not None and len(boxes_conf) > idx
                                    else float(np.mean(kpts_conf))
                                )
            ...
            results_dict = self.postprocess(
                (keypoints, scores, boxes, box_scores, track_ids_list),
                frame.shape,
            )
    ...
    def postprocess(
        self,
        raw_results: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List],
        frame_shape: Tuple[int, int, int],
    ) -> Dict[str, Any]:
        """Postprocess YOLO pose detection results - keep top identities."""
        ...
        for i, person_scores in enumerate(scores):
            avg_confidence = float(np.mean(person_scores))
            ...
            if avg_confidence > self.conf_threshold:
                det = {
                    ...
                    "box_confidence": float(box_scores[i])
                    if box_scores is not None and len(box_scores) > i
                    else avg_confidence,
                }
        ...
        if top_detections:
            ...
            return {
                ...
                "confidence": top_detections[0]["confidence"],
                "track_ids": top_track_ids,
                "boxes": top_boxes,
                "box_confidences": top_box_confidences,
            }
        else:
            return {
                "keypoints": None,
            ...
            for start_idx, end_idx in skeleton_connections:
                try:
                    ...
                    if scores is not None:
                        if (
                            scores[person_idx, start_idx] <= self.conf_threshold
                            or scores[person_idx, end_idx] <= self.conf_threshold
                        ):
                            continue

                    if (
                        not np.isnan(start_x)
                        and not np.isnan(start_y)
                        and np.isfinite(start_x)
                        and np.isfinite(start_y)
                        and not np.isnan(end_x)
                        and not np.isnan(end_y)
